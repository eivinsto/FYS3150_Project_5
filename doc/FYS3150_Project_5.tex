...
where $\hat{x},\hat{y} \in [0,1]$ are dimensionless parameters. This implies that $\alpha a_x$ and $\alpha a_y$ has units of length. The heat equation in two dimensions can then be written as:
...
where $D = k/(\rho c_p)$. We can multiply by $\alpha^2 a_x^2$ on both sides:

\begin{align*}
\frac{\partial^2 T}{\partial \hat{x}^2} + \frac{a_x^2}{a_y^2} \frac{\partial^2 T}{\partial \hat{y}^2} &= \alpha^2 a_x^2 D \frac{\partial T}{\partial t} - \frac{Q}{k}\alpha^2 a_x^2
\end{align*}

We now wish to make $\alpha^2 a_x^2 D = 1$, which means that $\alpha^2 = 1/(D\alpha^2)$. Inserting this gives:

\begin{align*}
\frac{\partial^2 T}{\partial \hat{x}^2} + \frac{a_x^2}{a_y^2} \frac{\partial^2 T}{\partial \hat{y}^2} &=  \frac{\partial T}{\partial t} - \frac{Q}{\rho c_p}
\end{align*}

Here we note that by this choice of $\alpha^2 a_x^2$ that $\alpha^2 a_x^2$ has the units as the constant $1/D$, which has units of m$^2$/s. This is not the unit that we originally defined this constant to have, but we can "fix" this by making time a dimensionless parameter. In order to that we can simply scale it against the unit we choose for time. By doing this $\alpha a_x$ has units of length as we originally defined. In order to simplify the notation we define:
...
\begin{align*}
\frac{\partial^2 T}{\partial \hat{x}^2} + A \frac{\partial^2 T}{\partial \hat{y}^2} &=  \frac{\partial T}{\partial t} - Q'
\end{align*}
...
\begin{align*}
\delta_{i,j}^l &= T_{i+1}^l + T_{i-1,j}^l + A(T_{i,j+1} + T_{i,j-1}) \, ,
\end{align*}

we can write the discretized heat equation as:

\begin{align*}
\frac{\Delta_{i,j}^l - 2(1+A)T_{i,j}^l}{h^2} &= \frac{T_{i,j}^l - T_{i,j}^{l-1}}{\Delta t} - Q' \\
\beta (\Delta_{i,j}^l - 2(1+A)T_{i,j}^l) &= T_{i,j}^l - T_{i,j}^{l-1} - \Delta t Q' \\
T_{i,j}^l &= \frac{1}{1 + 2\beta (1+A) } (\beta \Delta_{i,j}^l + T_{i,j}^{l-1} + \Delta t Q') \numberthis \label{eq:heat_eq_with_source_term_numerical_solution}
\end{align*}
...
the "diagonal" elements to be $1 + 2\beta(1+A)$ and the "off-diagonal" elements to be either $-\beta$ or $-\beta A$ (two elements of each value). The solution still converges, as the absolute of the "diagonal" elements are still larger than the absolute of the "off-diagonal" elements added together:

\begin{align*}
|1 + 2\beta(1+A)| > |-2\beta| + |-2\beta A|
\end{align*}

This equality always holds true, as both $A$ and $\beta$ are positive numbers. This means that we can use the same iterative solver as we did earlier, assuming that it is sufficiently generalized.
