...
Note that the first and last row of the matrix are set to zero. This is to make sure we do not manipulate the boundaries, which are assumed to be fixed as functions of time. As they may be non-zero, they are necessary in order to calculate the solution at the other points and cannot be safely excluded from the matrix-vector equation. Also note that if we know the solution at a previous timestep $U_j$ we can directly determine the solution in the next timestep $U_{j+1}$, and thus we do not need to solve this matrix-vector equation per se. It is just a practical way of denoting things that we will get back to.
...
This can be rewritten as a matrix-vector equation using the identity matrix $I$, the matrix:
...
and the vector $U_j$ which was defined earlier:
