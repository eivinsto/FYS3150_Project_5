...
When testing the two-dimensional solver we have implemented and used the error as defined in equation \eqref{eq:error_2D}. The initial state is given by a product of sinusoidals:

\begin{align*}
g(x,y) &= \sin(2\pi x) \sin (2\pi y) \, ,
\end{align*}

where we also assume Dirichlet boundary conditions. In this case, according to the results in Section \ref{sec:formalism_2D_diff_eq_analytical}, the analytical solution is:

\begin{align*}
u(x,y,t) &= \sin(2\pi x) \sin(2\pi y) e^{-8\pi^2 t} \, ,
\end{align*}

which we will compare with the numerical result.
...
\caption{•} \label{fig:forward_euler_solution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{../data/BackwardEuler.pdf}
\caption{•} \label{fig:backward_euler_solution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{../data/CrankNicholson.pdf}
\caption{•} \label{fig:crank_nicolson_solution}
\end{figure}
...
\caption{•} \label{fig:lithosphere_before_enrichment}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{../data/enriched2Dheat.pdf}
\caption{•} \label{fig:lithosphere_after_enrichment}
\end{figure}
